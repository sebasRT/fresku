import { FruverProduct, fruverSchema } from "@fresku/model/products/fruver";
...
function safeParseFruverProducts(rawProducts: any[]) {
    return rawProducts
        ...
        .filter((product) => {
            const result = fruverSchema.safeParse(product);
            if (!result.success) {
...
export { FRUVER_CATEGORIES, FRUVER_SUBCATEGORIES, safeParseFruverProducts, UNIT_TO_GRAMS };
